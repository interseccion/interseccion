...
// ── Recepción de notificaciones push y clic en ellas ─────────────────────────
self.addEventListener('push', (event) => {
    let data = {};
    try { data = event.data ? event.data.json() : {}; }
    catch (_) { data = { title: 'La Intersección', body: event.data ? event.data.text() : '' }; }

    const title = data.title || 'La Intersección';
    const options = {
        body: data.body || '',
        icon: './icon-128.png',
        badge: './icon-128.png',
